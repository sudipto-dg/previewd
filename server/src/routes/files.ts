...
import type { FileItem, BrowseResponse, Config } from "../types/index.js";
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      ...
      const limit = Math.min(
        parseInt(request.query.limit || String(config.pagination.defaultLimit), 10),
      ...
      try {
        ...
        return reply
          .status(500)
          .send({ error: "Failed to browse directory" });
      }
    },
  );
      ...
      try {
        ...
        if (range && contentType.startsWith("video/")) {
          ...
          reply
            ...
            .header("Accept-Ranges", contentType.startsWith("video/") ? "bytes" : "none")
            .send(fileStream);
        }
      } catch (error) {
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      ...
      try {
        ...
        try {
          fastify.log.info({ path: validation.resolvedPath }, "Generating video preview clip");
          ...
          if (!fs.existsSync(previewPath)) {
            fastify.log.error({ path: previewPath }, "Preview clip file was not created");
            throw new Error("Preview clip file was not created");
          }
          
          const previewStats = fs.statSync(previewPath);
          fastify.log.info({ path: previewPath, size: previewStats.size }, "Preview clip generated");
          
          if (previewStats.size === 0) {
            fastify.log.warn({ path: previewPath }, "Preview clip is empty, serving original");
            previewPath = validation.resolvedPath;
          }
        } catch (error) {
          fastify.log.error({ err: error, path: validation.resolvedPath }, "Failed to generate preview clip, serving original");
        ...
        if (range) {
          ...
          if (fileSize < 10 * 1024 * 1024) { // Less than 10MB
            ...
            fileStream.on("error", (err) => {
              fastify.log.error({ err, path: previewPath }, "Video preview stream error");
              if (!reply.sent) {
