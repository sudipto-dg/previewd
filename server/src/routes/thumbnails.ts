...
import { getCache, getConfig } from "../utils/configLoader.js";
...
export default async function thumbnailRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      ...
      const width = parseInt(request.query.width || "300", 10);
      const height = parseInt(request.query.height || "300", 10);
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      ...
      if (rawPaths.length > 0) {
        ...
        paths = Array.isArray(pathsArray) ? (pathsArray as string[]) : [pathsArray as string];
      }
      ...
      const width = parseInt((query.width as string) || "300", 10);
      const height = parseInt((query.height as string) || "300", 10);
      ...
      for (const chunk of chunks) {
        const promises = chunk.map(async (filePath) => {
          const validation = validatePath(filePath, config);
          if (!validation.valid || !validation.resolvedPath) {
            fastify.log.warn({ path: filePath, error: validation.error }, "Path validation failed");
          ...
          try {
            ...
            fastify.log.info({ path: filePath, thumbnailLength: thumbnail.length }, "Thumbnail generated");
            return { path: filePath, thumbnail };
          } catch (error) {
            fastify.log.warn({ err: error, path: filePath }, "Failed to generate thumbnail");
            return { path: filePath, thumbnail: null };
          }
        });
      ...
      fastify.log.info({ thumbnailCount: Object.keys(thumbnails).length }, "Batch thumbnails response");
