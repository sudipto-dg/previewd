...
function ThumbnailItem({ item, size, onClick }: ThumbnailItemProps) {
  ...
  const isVideo = item.type === "file" && item.extension && VIDEO_EXTENSIONS.includes(item.extension.toLowerCase());
  const isImage = item.type === "file" && item.extension && IMAGE_EXTENSIONS.includes(item.extension.toLowerCase());
  ...
  const videoPreviewUrl = isVideo
    ...
    : null;
  const fileUrl = item.type === "file" && !isVideo
    ...
    const loadVideo = async () => {
      ...
      try {
        ...
        if (expectedSize && arrayBuffer.byteLength !== expectedSize) {
          console.warn(`Size mismatch: expected ${expectedSize}, got ${arrayBuffer.byteLength}`);
        }

        // Convert arrayBuffer to blob
        const blob = new Blob([arrayBuffer], { type: response.headers.get("content-type") || "video/mp4" });
    ...
    >
      <div className="thumbnail-content">
        {item.type === "directory" ? (
          ...
          videoLoading ? (
            <div className="thumbnail-placeholder" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span>Loading...</span>
            </div>
          ) : videoUrl && !videoError ? (
            <video
              ...
              style={{ pointerEvents: "auto", cursor: "pointer", objectFit: "cover" }}
              muted
              playsInline
              autoPlay
              loop
